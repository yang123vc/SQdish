...
class Cart
{
	...
	*/
	...
	private function getGoodInfo($gid)
	{
		...
		$dataArray = array();
		$db = new DB();
		$dataArray = $db->select_one("select id,dishNameCN,count as shuliang,price as cost,pic as img,sid as shopid,sellcount from f_dish goods where id=".$gid."  ");
	...
	private function cartFormat($cartValue)
	{
		...
		if(count($cartValue) > 0)
		{
			...
			$shopinfo= array();
			if(!empty($ids)){
				$shoplist = $db->get_all("select id,sellerName,QRCode  from  f_seller   where id in (".$ids.")  ");
		...
		if($goodsIdArray)
		{
			...
			$goodsData = $db->get_all("select id,name,count as shuliang,price as cost,pic as img, sid as shopid,sellcount, dtid as typeid  from f_dish  where id in (".join(",",$goodsIdArray).")  order by id desc  ");
			...
			foreach($goodsData as $key => $val)
			{
				...
				$backdata['list'][$val['shopid']]['data'][$val['id']]['bagcost'] = 0;
