...
function getarticleurl($r){
	global $isw;
	if(!isset($isw)){$isw=0;}
	$t=date("Ymd",$r['addtime']);
	if($isw){
		$url="/a/$t/{$r['id']}.html";
	}else{
		$url="/article_detail.php?tid={$r['id']}&t=$t";
	}
	return $url;	
}

function getcategoryurl($r){
	global $isw;
	if(!isset($isw)){$isw=0;}
	//$t=date("Ymd",$r['addtime']);
	if($isw){
		$url="/a/list_{$r['id']}/index.html";
	}else{
		$url="/article_list.php?tid={$r['id']}";
	}
	return $url;	
}
...
function getpre($r){
	global $db;
	$nowid=$r['id'];
	$sql = "select * from f_article where id < $nowid order by id desc; ";		
	$nr  = $db->get_one($sql);	
	if(empty($nr)){
		return "no pre!";
	}else{
		$url=getarticleurl($nr);
		return "<a href='$url'>{$nr['articleTitle']}</a>";
	}	
}
...
function getnext($r){
	global $db;
	$nowid=$r['id'];
	$sql = "select * from f_article where id > $nowid order by id asc;";
	$nr  = $db->get_one($sql);	
	if(empty($nr)){
		return "no next!";
	}else{
