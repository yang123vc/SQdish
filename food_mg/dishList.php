<?php
	...
	$sql = "select * from f_dish where sid = $sid $where order by id desc limit $location,$pageSize";
	$dishList = $db->get_all($sql);
	
	$sql = "select count(*) as count from f_dish where sid = $sid $where order by id desc";
	$temp = $db->get_one($sql);
	$count = $temp['count'];
