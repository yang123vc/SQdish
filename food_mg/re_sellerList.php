<?php
	...
	$sql = "select id,sellerName,tel,contacts,QRCode,dishPic,path,is_on,_error from f_seller where referee='$id' order by id desc limit $location,$pageSize";
	...
	$sql = "select count(*) as count from f_seller where referee='$id'";
	$temp = $db->get_one($sql);
	$count = $temp['count'];
