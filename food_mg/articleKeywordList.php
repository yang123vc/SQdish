<?php
	...
	if($keywords){
		$where = "where KeywordName like '%$keywords%'";
	}
	//分页参数
	$pageSize = 10;
	...
	$sql = "select * from f_article_keywords $where order by id desc limit $location,$pageSize";
	$List = $db->get_all($sql);
	
	$sql = "select count(*) as count from f_article_keywords $where";
	$temp = $db->get_one($sql);
	$count = $temp['count'];
	...
	include_once dirname(__FILE__)."/templates/articleKeywordList.html";
?>