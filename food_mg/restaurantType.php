<?php
	...
	$sql = "select * from f_dish_rtype where sid=0 order by rt_id desc limit $location,$pageSize";
	$dishList = $db->get_all($sql);
	
	$sql = "select count(*) as count from f_dish_rtype where sid =0 $where order by rt_id desc";
	$temp = $db->get_one($sql);
	$count = $temp['count'];
	...
	include_once dirname(__FILE__)."/templates/restaurantType.html";
?>